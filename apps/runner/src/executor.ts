import { chromium, expect, Page, Request } from '@playwright/test'
...
export async function runTestCase({ runId, testCase, environment, dataset }: RunInput): Promise<void> {
  ...
  page.on('response', response => {
    ...
    if (!shouldCaptureNetworkEntry(request.url(), request.resourceType())) return
...
async function saveStepScreenshot(page: Page, runId: string, stepId: string): Promise<string | undefined> {
  try {
...
function shouldCaptureNetworkEntry(url: string, resourceType: string): boolean {
...
function compactConsoleLogs(logs: ConsoleLogEntry[]): ConsoleLogEntry[] {
  return logs.slice(-120)
}

function compactNetworkLogs(logs: NetworkLogEntry[]): NetworkLogEntry[] {
