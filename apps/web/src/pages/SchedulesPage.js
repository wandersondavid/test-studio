...
export function SchedulesPage() {
    ...
    function load() {
        ...
        Promise.all([
            schedulesApi.list(),
            api.get('/test-cases').then(r => r.data),
    ...
    return (<div data-testid="schedules-page" className="page-shell">
      <PageHeader eyebrow="Automação" title="Agendamentos" description="Configure execuções automáticas de cenários por expressão cron." actions={<button className="button-primary" data-testid="btn-new-schedule" onClick={openNew}>
            + Novo agendamento
          </button>} meta={<>
      ...
      {showForm && (<form data-testid="schedule-form" onSubmit={handleSubmit} className="surface inline-form">
          ...
          <div className="field-grid">
            <label className="field">
              ...
              <input data-testid="input-name" placeholder="Ex: Smoke test diário" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} required/>
            </label>

            <label className="field">
              <span className="field-label">Expressão cron</span>
              <input data-testid="input-cron" placeholder="0 * * * *" value={form.cron} onChange={e => setForm(f => ({ ...f, cron: e.target.value }))} required/>
            ...
            <label className="field">
              ...
              <select data-testid="select-case" value={form.caseId} onChange={e => setForm(f => ({ ...f, caseId: e.target.value }))} required>
            ...
            <label className="field">
              ...
              <select data-testid="select-environment" value={form.environmentId} onChange={e => setForm(f => ({ ...f, environmentId: e.target.value }))} required>
                <option value="">Selecione um ambiente...</option>
                {environments.map(env => (<option key={env._id} value={env._id}>{env.name}</option>))}
              </select>
            </label>

            <label className="field">
              <span className="field-label">Dataset (opcional)</span>
              <select data-testid="select-dataset" value={form.datasetId ?? ''} onChange={e => setForm(f => ({ ...f, datasetId: e.target.value || undefined }))}>
            ...
            <label className="field" style={{ flexDirection: 'row', alignItems: 'center', gap: '0.75rem' }}>
              <input type="checkbox" data-testid="toggle-active" checked={form.isActive ?? true} onChange={e => setForm(f => ({ ...f, isActive: e.target.checked }))}/>
      ...
      {schedules.length === 0 ? (<div className="empty-state" data-testid="empty">Nenhum agendamento cadastrado.</div>) : (<section className="surface">
          <div className="section-heading">
            <div>
              <h2>Agendamentos</h2>
          ...
          <div className="table-shell">
            <table className="table">
              ...
              <tbody>
                {schedules.map(s => (<tr key={s._id} data-testid={`schedule-row-${s._id}`}>
                    <td>{s.name}</td>
                    ...
                    <td>
                      <div className="flex gap-2" style={{ display: 'flex', gap: '0.5rem' }}>
                        <button className="button-secondary" data-testid={`btn-edit-${s._id}`} onClick={() => openEdit(s)}>
                          Editar
                        </button>
                        <button className="button-primary" data-testid={`btn-trigger-${s._id}`} disabled={triggeringId === s._id} onClick={() => handleTrigger(s._id)}>
                          {triggeringId === s._id ? 'Disparando...' : 'Executar agora'}
                        </button>
                        <button className="button-danger" data-testid={`btn-delete-${s._id}`} onClick={() => setConfirmDelete({ id: s._id, name: s.name })}>
                          Deletar
                        </button>
                      </div>
                    </td>
      ...
      {confirmDelete && (<ConfirmDialog open={!!confirmDelete} title="Deletar agendamento" description={`Tem certeza que deseja deletar "${confirmDelete.name}"?`} tone="danger" confirmLabel="Deletar" onConfirm={() => {
