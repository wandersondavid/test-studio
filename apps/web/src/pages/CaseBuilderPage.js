...
import { buildParameterValueMap, collectTemplateVariables, interpolateStepsWithVariables, } from '../lib/blockInterpolation';
const STEP_TYPES = [
    'visit',
    'click',
    'fill',
...
function stepNeedsSelector(type) {
    return !['visit', 'waitForURL', 'waitForApi'].includes(type);
}
function stepNeedsValue(type) {
    return !['click', 'check', 'assertVisible', 'waitForApi'].includes(type);
}
function stepNeedsApi(type) {
    return type === 'waitForApi';
}
function actionNeedsValue(action) {
    return ['fill', 'select'].includes(action);
}
function actionInputLabel(action) {
    if (action === 'fill')
        return 'Texto para preencher';
    if (action === 'select')
        return 'Value da option';
    return 'Texto esperado';
}
function actionInputPlaceholder(action) {
    if (action === 'fill')
        return 'ex: admin@empresa.com';
    if (action === 'select')
        return 'ex: manager';
...
function describeStep(step) {
    ...
    if (step.selector && maskedValue)
        return `${label}: ${step.selector} → "${maskedValue}"${timingSummary}`;
    if (step.selector)
        return `${label}: ${step.selector}${timingSummary}`;
    if (maskedValue)
        return `${label}: ${maskedValue}${timingSummary}`;
    if (step.description)
        return `${label}: ${step.description}${timingSummary}`;
    return `${label}${timingSummary}`;
}
function isSensitiveFieldIdentifier(value) {
    if (!value)
        return false;
    return /(password|senha|current-password|new-password)/i.test(value);
}
function shouldMaskStepValue(step) {
    if (step.type !== 'fill')
        return false;
...
function buildTimingDraft(step) {
    return {
...
function normalizeRetryConfig(attemptsRaw, intervalSecondsRaw) {
...
function normalizeApiCondition(api) {
    if (!api)
        return undefined;
...
function mergeApiDraft(current, patch) {
    return {
        ...current,
        api: {
...
function describeStepTiming(step) {
    const parts = [];
    if (step.timeoutMs) {
...
export function CaseBuilderPage() {
    ...
    const [newStep, setNewStep] = useState({ type: 'click' });
    const [newStepTiming, setNewStepTiming] = useState(() => buildTimingDraft());
    const [showAddForm, setShowAddForm] = useState(false);
    const [editingStepId, setEditingStepId] = useState(null);
    const [editingTiming, setEditingTiming] = useState(() => buildTimingDraft());
    ...
    const [blockInsertTarget, setBlockInsertTarget] = useState(null);
    const [blockParameterValues, setBlockParameterValues] = useState({});
    ...
    const [recorderViewport, setRecorderViewport] = useState(null);
    const [screenshotVersion, setScreenshotVersion] = useState(0);
    const [recorderScreenshotUrl, setRecorderScreenshotUrl] = useState(null);
    const [loadingRecorderScreenshot, setLoadingRecorderScreenshot] = useState(false);
    const [recorderPreviewError, setRecorderPreviewError] = useState(null);
    const [recorderMode, setRecorderMode] = useState('auto');
    const [recorderAction, setRecorderAction] = useState('click');
    const [recorderActionValue, setRecorderActionValue] = useState('');
    const [pendingAutoInput, setPendingAutoInput] = useState(null);
    const [autoInputDraft, setAutoInputDraft] = useState('');
    const [showSensitiveAutoInput, setShowSensitiveAutoInput] = useState(false);
    const [recorderOverlayPosition, setRecorderOverlayPosition] = useState(null);
    ...
    const recorderOverlayRef = useRef(null);
    const recorderSessionRef = useRef(null);
    const recorderScreenshotUrlRef = useRef(null);
    const loadingRecorderScreenshotRef = useRef(false);
    const autoInputRef = useRef(null);
    const recorderDragStateRef = useRef(null);
    ...
    function clampRecorderOverlayPosition(next) {
    ...
    function placeRecorderOverlayDefault() {
        ...
        const overlayWidth = recorderOverlayRef.current?.offsetWidth ?? 360;
        const margin = 18;
        setRecorderOverlayPosition(clampRecorderOverlayPosition({
            x: Math.max(margin, container.clientWidth - overlayWidth - margin),
            y: margin,
        }));
    }
    function placeRecorderOverlayNearPreviewPoint(pointX, pointY) {
        setRecorderOverlayPosition(clampRecorderOverlayPosition({
            x: pointX + 18,
            y: pointY - 12,
        }));
    ...
    async function loadReusableBlocks() {
        ...
        try {
            const response = await api.get('/reusable-blocks');
    ...
    useEffect(() => {
        ...
        void (async () => {
            try {
                const response = await fetch(`/runner/recorder/sessions/${recorderSessionId}/screenshot?ts=${screenshotVersion}`, {
                    headers: {
                ...
                if (cancelled)
                    return;
                resetRecorderScreenshot();
                setRecorderPreviewError(err instanceof Error
                    ? err.message
    ...
    async function saveSteps(nextSteps) {
        ...
        setError(null);
        try {
            const response = await api.put(`/test-cases/${id}`, { steps: nextSteps });
    ...
    async function saveCasePatch(patch) {
        ...
        setError(null);
        try {
            const response = await api.put(`/test-cases/${id}`, patch);
    ...
    function appendSteps(recordedSteps) {
    ...
    function applyTimingPreset(target, preset) {
        if (target === 'new') {
    ...
    function updateBlockParameter(key, patch) {
        setBlockParameters(current => current.map(parameter => (parameter.key === key
            ? { ...parameter, ...patch }
            : parameter)));
    }
    ...
    function handleInsertReusableBlock(block) {
        if (block.parameters && block.parameters.length > 0) {
            setBlockInsertTarget(block);
            setBlockParameterValues(Object.fromEntries(block.parameters.map(parameter => [parameter.key, parameter.defaultValue ?? ''])));
            return;
        }
    ...
    async function handleSaveSetupCase() {
        ...
        try {
            ...
            if (updated) {
                setRecorderStatus(updated.setupCaseId
    ...
    async function applySetupCaseToRecorderSession(sessionId) {
        if (!selectedSetupCaseId) {
            return false;
        }
        const response = await api.get(`/test-cases/${selectedSetupCaseId}/executable`);
        const session = await runnerRequest(`/recorder/sessions/${sessionId}/replay`, {
            method: 'POST',
            body: JSON.stringify({
    ...
    function openStepTimingEditor(step) {
        setEditingStepId(step.id);
        setEditingTiming(buildTimingDraft(step));
        setError(null);
    }
    function handleSaveStepTiming(stepId) {
        let timeoutMs;
        let retry;
        try {
        ...
        replaceSteps(stepsRef.current.map(step => step.id === stepId
            ? {
                ...step,
                timeoutMs,
                retry,
    ...
    function applyRecorderState(session) {
    ...
    async function commitPendingAutoInputIfNeeded() {
        ...
        setRecorderStatus(pendingAutoInput.action === 'select'
            ? 'Aplicando seleção automática antes da próxima ação...'
            : 'Aplicando preenchimento automático antes da próxima ação...');
        const session = await runnerRequest(`/recorder/sessions/${recorderSessionRef.current}/type`, {
            method: 'POST',
            body: JSON.stringify({
    ...
    async function createRecorderSession(options) {
        ...
        const session = await runnerRequest('/recorder/sessions', {
            method: 'POST',
            body: JSON.stringify({
                environment,
                startPath,
        ...
        if (options?.replaySteps && options.replaySteps.length > 0) {
            ...
            const replayedSession = await runnerRequest(`/recorder/sessions/${session.sessionId}/replay`, {
                method: 'POST',
                body: JSON.stringify({
    ...
    async function handleRecordFromStep(targetIndex) {
        setRecorderBusy(true);
        setError(null);
    ...
    async function handleRefreshRecorder() {
        ...
        setRecorderBusy(true);
        try {
            const session = await runnerRequest(`/recorder/sessions/${recorderSessionRef.current}`);
            applyRecorderState(session);
    ...
    async function handleOpenPath() {
        ...
        setError(null);
        try {
            const session = await runnerRequest(`/recorder/sessions/${recorderSessionRef.current}/navigate`, {
                method: 'POST',
    ...
    async function handlePreviewClick(event) {
        if (!recorderSessionRef.current || recorderBusy)
            return;
        ...
        try {
            ...
            const session = await runnerRequest(`/recorder/sessions/${recorderSessionRef.current}/interact`, {
                method: 'POST',
                body: JSON.stringify({
                    action,
                    x,
            ...
            if (recorderMode === 'auto' && session.pendingInput) {
                ...
                setRecorderStatus(session.pendingInput.action === 'select'
    ...
    if (!testCase)
        return <div className="empty-state" data-testid="not-found">Cenário não encontrado</div>;
    ...
    const recorderOverlayStyle = recorderOverlayPosition
        ...
        : undefined;
    return (<div data-testid="case-builder-page" className="page-shell">
      <PageHeader eyebrow="Builder visual" title={testCase.name} description="Monte steps manualmente ou grave interações reais em uma sessão Playwright sem sair do workspace." actions={<>
            ...
            <Button variant={showAddForm ? 'outline' : 'default'} data-testid="btn-open-add-step-header" onClick={() => setShowAddForm(current => !current)}>
      ...
      {error && (<div className="alert alert-error" data-testid="builder-error">
          {error}
        </div>)}

      {showRecorder && (<Card data-testid="recorder-panel" className="recorder-shell bg-card/70">
          ...
          {loadingEnvironments ? (<CardContent>
              <div className="loading-state">Carregando ambientes...</div>
            </CardContent>) : environments.length === 0 ? (<CardContent>
              ...
              <div className="field-grid">
                <label className="field">
                  ...
                  <select data-testid="select-recorder-environment" value={recorderEnvironmentId} onChange={event => setRecorderEnvironmentId(event.target.value)} disabled={recorderBusy}>
                    {environments.map(environment => (<option key={environment._id} value={environment._id}>
                ...
                <label className="field">
                  ...
                  <input data-testid="input-recorder-path" placeholder="/login" value={recorderPath} onChange={event => setRecorderPath(event.target.value)} disabled={recorderBusy}/>
                </label>
              </div>
              ...
              <div className="alert alert-info">
                ...
                {recorderMode === 'auto' ? (<div className="muted" style={{ marginTop: 8 }}>
                ...
                {pageTitle && (<div className="muted" style={{ marginTop: 8 }}>
                    Título da página: <code>{pageTitle}</code>
                  </div>)}
                {lastCapturedSelector && (<div className="muted" style={{ marginTop: 8 }}>
                    Último seletor capturado: <code>{lastCapturedSelector}</code>
                  </div>)}
                {recorderViewport && (<div className="muted" style={{ marginTop: 8 }}>
              ...
              <div className="recorder-preview" ref={previewFrameRef}>
                {recorderSessionId && (<div ref={recorderOverlayRef} className="recorder-overlay" style={recorderOverlayStyle} data-testid="recorder-floating-panel">
                    <div className="recorder-overlay-header">
                      <button type="button" className="recorder-overlay-handle" onPointerDown={handleRecorderOverlayPointerDown} aria-label="Arrastar painel do recorder">
                    ...
                    <div className="recorder-overlay-mode-switch">
                      <Button type="button" size="sm" variant={recorderMode === 'auto' ? 'default' : 'outline'} data-testid="btn-recorder-mode-auto" onClick={() => setRecorderMode('auto')} disabled={recorderBusy}>
                        Automático
                      </Button>
                      <Button type="button" size="sm" variant={recorderMode === 'manual' ? 'default' : 'outline'} data-testid="btn-recorder-mode-manual" onClick={() => setRecorderMode('manual')} disabled={recorderBusy}>
                        Manual
                      </Button>
                    </div>

                    <div className="recorder-overlay-body">
                      {recorderMode === 'auto' ? (pendingAutoInput ? (<>
                            <label className="field recorder-overlay-field">
                              <span className="field-label">
                                {pendingAutoInput.action === 'select' ? 'Value da option' : 'Preencher campo'}
                                {sensitiveAutoInput && (<Badge variant="outline" className="ml-2">Sensível</Badge>)}
                              </span>
                              <div className="recorder-overlay-input-row">
                                <input ref={autoInputRef} type={sensitiveAutoInput && !showSensitiveAutoInput ? 'password' : 'text'} data-testid="input-recorder-auto-value" placeholder={pendingAutoInput.action === 'select'
                            ? 'ex: manager'
                            : pendingAutoInput.description
                                ? `Digite para ${pendingAutoInput.description}`
                                : 'Digite o valor do campo'} value={autoInputDraft} onChange={event => setAutoInputDraft(event.target.value)} onKeyDown={event => {
                            if (event.key === 'Enter') {
                                event.preventDefault();
                                void handleAutoInputSubmit();
                            }
                        }} disabled={recorderBusy}/>
                                {sensitiveAutoInput && (<Button type="button" size="sm" variant="outline" data-testid="btn-toggle-sensitive-auto-input" onClick={() => setShowSensitiveAutoInput(current => !current)} disabled={recorderBusy}>
                            ...
                            <div className="recorder-overlay-actions">
                              <Button type="button" size="sm" data-testid="btn-recorder-auto-apply" onClick={() => void handleAutoInputSubmit()} disabled={recorderBusy}>
                                Aplicar
                              </Button>
                          ...
                          <label className="field recorder-overlay-field">
                            ...
                            <select data-testid="select-recorder-action" value={recorderAction} onChange={event => setRecorderAction(event.target.value)} disabled={recorderBusy}>
                              {RECORDER_ACTIONS.map(action => (<option key={action} value={action}>
                                  {RECORDER_ACTION_LABELS[action]}
                                </option>))}
                            </select>
                          </label>

                          {(actionNeedsValue(recorderAction) || recorderAction === 'assertText') && (<label className="field recorder-overlay-field">
                              <span className="field-label">{actionInputLabel(recorderAction)}</span>
                              <input data-testid="input-recorder-action-value" placeholder={actionInputPlaceholder(recorderAction)} value={recorderActionValue} onChange={event => setRecorderActionValue(event.target.value)} disabled={recorderBusy}/>
                            </label>)}
                ...
                {recorderScreenshotUrl ? (<img ref={previewImageRef} src={recorderScreenshotUrl} alt="Preview da sessão Playwright" data-testid="recorder-preview-image" onClick={handlePreviewClick} style={{ cursor: recorderBusy ? 'progress' : 'crosshair', userSelect: 'none' }}/>) : recorderSessionId ? (<div className="empty-state">
      ...
      <Card className="bg-card/70">
        ...
        <CardContent className="space-y-4">
          {loadingAvailableCases ? (<div className="loading-state">Carregando cenários disponíveis...</div>) : availableCases.length === 0 ? (<div className="empty-state">
              Crie pelo menos outro cenário para usar como base reutilizável.
            </div>) : (<>
              <div className="field-grid">
                <label className="field">
                  <span className="field-label">Cenário base / setup</span>
                  <select data-testid="select-setup-case" value={selectedSetupCaseId} onChange={event => setSelectedSetupCaseId(event.target.value)} disabled={savingSetupCase || applyingSetupCase}>
                    <option value="">Sem cenário base</option>
                    {availableCases.map(testCaseOption => (<option key={testCaseOption._id} value={testCaseOption._id}>
                        {testCaseOption.name}
                      </option>))}
                  </select>
                </label>
              ...
              <div className="form-actions">
                <Button data-testid="btn-save-setup-case" onClick={() => void handleSaveSetupCase()} disabled={savingSetupCase}>
                  {savingSetupCase ? 'Salvando setup...' : 'Salvar cenário base'}
                </Button>
                <Button variant="outline" data-testid="btn-apply-setup-case" onClick={() => void handleApplySetupCaseToRecorder()} disabled={!selectedSetupCaseId || !recorderSessionId || applyingSetupCase || recorderBusy}>
      ...
      <Card className="bg-card/70">
        ...
        <CardContent className="space-y-5">
          <ol className="step-list">
            {steps.map((step, index) => (<li key={step.id} data-testid={`step-item-${step.id}`}>
                <div className="step-card">
                  <div className="step-card-main">
                    ...
                    {(step.timeoutMs || step.retry) && (<div className="mt-3 flex flex-wrap gap-2">
                        {step.timeoutMs && (<Badge variant="outline">timeout {formatSeconds(step.timeoutMs)}s</Badge>)}
                        {step.retry && (<Badge variant="secondary">
                            {step.retry.attempts} tentativas / {formatSeconds(step.retry.intervalMs)}s
                          </Badge>)}
                      </div>)}
                    {step.selectorAlternatives && step.selectorAlternatives.length > 0 && (<div className="mt-3 flex flex-wrap gap-2">
                  ...
                  <div className="step-actions">
                    <Button variant="outline" size="sm" data-testid={`btn-record-from-${step.id}`} onClick={() => handleRecordFromStep(index)} disabled={loadingEnvironments || environments.length === 0 || recorderBusy}>
                      Gravar daqui
                    </Button>
                    <Button variant="ghost" size="sm" data-testid={`btn-config-${step.id}`} onClick={() => editingStepId === step.id
                ? setEditingStepId(null)
                : openStepTimingEditor(step)}>
                      {editingStepId === step.id ? 'Fechar' : 'Retry'}
                    </Button>
                    <Button variant="outline" size="icon" data-testid={`btn-up-${step.id}`} onClick={() => moveStep(index, -1)} disabled={index === 0}>
                      ↑
                    </Button>
                    <Button variant="outline" size="icon" data-testid={`btn-down-${step.id}`} onClick={() => moveStep(index, 1)} disabled={index === steps.length - 1}>
                      ↓
                    </Button>
                    <Button variant="destructive" size="icon" data-testid={`btn-remove-${step.id}`} onClick={() => handleRemoveStep(step.id)}>
                      ✕
                    </Button>
                  </div>
                </div>
                {editingStepId === step.id && (<div className="mt-4 rounded-xl border border-border/70 bg-background/70 p-4">
                    ...
                    <div className="field-grid">
                      <label className="field">
                        ...
                        <input data-testid={`input-timeout-${step.id}`} placeholder="10" value={editingTiming.timeoutSeconds} onChange={event => setEditingTiming(current => ({ ...current, timeoutSeconds: event.target.value }))}/>
                      </label>

                      <label className="field">
                        <span className="field-label">Tentativas totais</span>
                        <input data-testid={`input-retry-attempts-${step.id}`} placeholder="1" value={editingTiming.retryAttempts} onChange={event => setEditingTiming(current => ({ ...current, retryAttempts: event.target.value }))}/>
                      </label>

                      <label className="field">
                        <span className="field-label">Intervalo entre tentativas (segundos)</span>
                        <input data-testid={`input-retry-interval-${step.id}`} placeholder="10" value={editingTiming.retryIntervalSeconds} onChange={event => setEditingTiming(current => ({ ...current, retryIntervalSeconds: event.target.value }))}/>
                      </label>
                    </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {RETRY_PRESETS.map(preset => (<Button key={preset.key} variant="outline" size="sm" onClick={() => applyTimingPreset('edit', preset)}>
                        {preset.label}
                      </Button>))}
                  </div>

                  <div className="form-actions" style={{ marginTop: 16 }}>
                      <Button size="sm" data-testid={`btn-save-config-${step.id}`} onClick={() => handleSaveStepTiming(step.id)}>
                        Salvar config
                      </Button>
          ...
          {showAddForm ? (<div data-testid="add-step-form" className="surface surface-muted">
              ...
              <div className="field-grid">
                <label className="field">
                  ...
                  <select data-testid="select-step-type" value={newStep.type} onChange={event => setNewStep(current => ({ ...current, type: event.target.value }))}>
                    {STEP_TYPES.map(type => (<option key={type} value={type}>
                        {STEP_LABELS[type]}
                      </option>))}
                  </select>
                </label>

                {stepNeedsSelector(newStep.type ?? 'click') && (<label className="field">
                    <span className="field-label">Seletor CSS</span>
                    <input data-testid="input-step-selector" placeholder='[data-testid="meu-elemento"]' value={newStep.selector ?? ''} onChange={event => setNewStep(current => ({ ...current, selector: event.target.value }))}/>
                  </label>)}

                {stepNeedsValue(newStep.type ?? 'click') && (<label className="field">
                    <span className="field-label">Valor</span>
                    <input data-testid="input-step-value" placeholder='valor ou {{variavel}}' value={newStep.value ?? ''} onChange={event => setNewStep(current => ({ ...current, value: event.target.value }))}/>
                  </label>)}

                {stepNeedsApi(newStep.type ?? 'click') && (<>
                    <label className="field">
                      <span className="field-label">URL contém</span>
                      <input data-testid="input-step-api-url" placeholder="/api/cargas" value={newStep.api?.urlContains ?? ''} onChange={event => setNewStep(current => mergeApiDraft(current, { urlContains: event.target.value }))}/>
                    </label>

                    <label className="field">
                      <span className="field-label">Método</span>
                      <input data-testid="input-step-api-method" placeholder="POST" value={newStep.api?.method ?? ''} onChange={event => setNewStep(current => mergeApiDraft(current, { method: event.target.value }))}/>
                    </label>

                    <label className="field">
                      <span className="field-label">Status esperado</span>
                      <input data-testid="input-step-api-status" placeholder="200" value={newStep.api?.status?.toString() ?? ''} onChange={event => setNewStep(current => mergeApiDraft(current, {
                    status: (() => {
                    ...
                    <label className="field">
                      ...
                      <input data-testid="input-step-api-response" placeholder="id ou status do backend" value={newStep.api?.responseIncludes ?? ''} onChange={event => setNewStep(current => mergeApiDraft(current, { responseIncludes: event.target.value }))}/>
                    </label>
                  </>)}

                <label className="field">
                  <span className="field-label">Timeout por tentativa (segundos)</span>
                  <input data-testid="input-step-timeout" placeholder="10" value={newStepTiming.timeoutSeconds} onChange={event => setNewStepTiming(current => ({ ...current, timeoutSeconds: event.target.value }))}/>
                </label>

                <label className="field">
                  <span className="field-label">Tentativas totais</span>
                  <input data-testid="input-step-retry-attempts" placeholder="1" value={newStepTiming.retryAttempts} onChange={event => setNewStepTiming(current => ({ ...current, retryAttempts: event.target.value }))}/>
                </label>

                <label className="field">
                  <span className="field-label">Intervalo entre tentativas (segundos)</span>
                  <input data-testid="input-step-retry-interval" placeholder="10" value={newStepTiming.retryIntervalSeconds} onChange={event => setNewStepTiming(current => ({ ...current, retryIntervalSeconds: event.target.value }))}/>
                </label>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {RETRY_PRESETS.map(preset => (<Button key={preset.key} variant="outline" size="sm" onClick={() => applyTimingPreset('new', preset)}>
                    {preset.label}
                  </Button>))}
              </div>
            ...
            </div>) : (<div className="form-actions" style={{ marginTop: 18 }}>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_420px]">
          ...
          <CardContent className="space-y-5">
            <div className="field-grid">
              <label className="field">
                ...
                <input value={blockForm.name} onChange={event => setBlockForm(current => ({ ...current, name: event.target.value }))} placeholder="Ex: Login base"/>
              </label>
              <label className="field">
                <span className="field-label">Descrição</span>
                <input value={blockForm.description} onChange={event => setBlockForm(current => ({ ...current, description: event.target.value }))} placeholder="Passos reutilizados em vários fluxos"/>
              </label>
            </div>
            ...
            <div className="rounded-xl border border-border/70 bg-background/50 p-4">
              ...
              {blockParameters.length === 0 ? (<div className="mt-4 text-sm text-muted-foreground">
                  Nenhum placeholder foi detectado nos steps atuais.
                </div>) : (<div className="mt-4 space-y-3">
                  {blockParameters.map(parameter => (<div key={parameter.key} className="rounded-xl border border-border/60 bg-background/60 p-3">
                      <div className="field-grid">
                        <label className="field">
                        ...
                        <label className="field">
                          ...
                          <input value={parameter.label ?? ''} onChange={event => updateBlockParameter(parameter.key, { label: event.target.value })} placeholder="Ex: Usuário operador"/>
                        </label>
                        <label className="field">
                          <span className="field-label">Valor padrão</span>
                          <input value={parameter.defaultValue ?? ''} onChange={event => updateBlockParameter(parameter.key, { defaultValue: event.target.value })} placeholder="Opcional"/>
                        </label>
                        <label className="field">
                          <span className="field-label">Descrição</span>
                          <input value={parameter.description ?? ''} onChange={event => updateBlockParameter(parameter.key, { description: event.target.value })} placeholder="Quando usar esse parâmetro"/>
                        </label>
                      </div>
                      <div className="mt-3 flex flex-wrap gap-2">
                        <Button type="button" size="sm" variant={parameter.required === false ? 'outline' : 'secondary'} onClick={() => updateBlockParameter(parameter.key, { required: parameter.required === false })}>
                          {parameter.required === false ? 'Opcional' : 'Obrigatório'}
                        </Button>
                        <Button type="button" size="sm" variant={parameter.secret ? 'secondary' : 'outline'} onClick={() => updateBlockParameter(parameter.key, { secret: !parameter.secret })}>
            ...
            {loadingBlocks ? (<div className="loading-state">Carregando blocos...</div>) : reusableBlocks.length === 0 ? (<div className="empty-state">Nenhum bloco salvo ainda.</div>) : (<div className="space-y-3">
                {reusableBlocks.map(block => (<div key={block._id} className="rounded-xl border border-border/70 bg-background/60 p-4">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        {block.parameters && block.parameters.length > 0 && (<div className="mt-2 flex flex-wrap gap-2">
                            {block.parameters.map(parameter => (<Badge key={parameter.key} variant={parameter.secret ? 'warning' : 'outline'}>
        ...
        <Card className="bg-card/70">
          ...
          <CardContent>
            {loadingAudit ? (<div className="loading-state">Carregando atividades...</div>) : auditLogs.length === 0 ? (<div className="empty-state">Nenhuma atividade registrada ainda.</div>) : (<div className="space-y-3">
                {auditLogs.map(entry => (<div key={entry._id} className="rounded-xl border border-border/70 bg-background/60 p-4">
                    <div className="text-sm font-semibold">{entry.summary}</div>
                    <div className="mt-2 text-xs text-muted-foreground">
                      {entry.actor.name} • {entry.actor.email}
                    </div>
                    {entry.metadata && (<div className="mt-2 flex flex-wrap gap-2">
                        {Object.entries(entry.metadata).map(([key, value]) => (<Badge key={key} variant="outline">
                            {key}: {String(value)}
                          </Badge>))}
                      </div>)}
                  </div>))}
      ...
      <ConfirmDialog open={Boolean(blockInsertTarget)} title={blockInsertTarget ? `Inserir bloco "${blockInsertTarget.name}"` : 'Inserir bloco reutilizável'} description="Preencha os parâmetros antes de anexar os steps ao cenário atual." confirmLabel="Inserir bloco" onCancel={() => {
            setBlockInsertTarget(null);
            setBlockParameterValues({});
        }} onConfirm={confirmInsertReusableBlock}>
        <div className="space-y-3">
          {blockInsertTarget?.parameters?.map(parameter => (<label key={parameter.key} className="field">
              <span className="field-label">{parameter.label || parameter.key}</span>
              <input type={parameter.secret ? 'password' : 'text'} value={blockParameterValues[parameter.key] ?? ''} onChange={event => setBlockParameterValues(current => ({
                ...current,
      ...
      <ExportModal open={showExportModal} caseId={id} caseName={testCase.name} onClose={() => setShowExportModal(false)}/>
      <AiStepsModal open={showAiModal} onClose={() => setShowAiModal(false)} onStepsGenerated={(newSteps) => setSteps(prev => [...prev, ...newSteps])}/>
    </div>);
}
//# sourceMappingURL=CaseBuilderPage.js.map