...
export function UsersPage() {
    ...
    if (user?.role !== 'admin') {
        return (<div className="empty-state" data-testid="users-forbidden">
        Apenas administradores podem acessar a gestão de usuários.
      </div>);
    }
    return (<div data-testid="users-page" className="page-shell">
      <PageHeader eyebrow="Administração" title="Usuários" description="Cadastre membros, ative ou desative acessos e mantenha a trilha de autoria das ações no Test Studio."/>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_380px]">
        <Card className="bg-card/70">
          ...
          <CardContent>
            {loading ? (<div className="loading-state">Carregando usuários...</div>) : (<div className="table-shell">
                <table className="table">
                  ...
                  <tbody>
                    {users.map(item => (<tr key={item._id}>
                        ...
                        <td>{item.status}</td>
                        <td>
                          <Button variant="outline" size="sm" onClick={() => toggleStatus(item)} disabled={saving || item._id === user._id}>
