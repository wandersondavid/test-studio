...
export function RunPage() {
    ...
    async function handleRun(e) {
        ...
        if (!selected.environmentId)
            return;
        if (mode === 'case' && !selected.caseId)
            return;
        if (mode === 'suite' && !selected.suiteId)
            return;
        setRunning(true);
        setError(null);
        setSuccess(null);
    ...
    return (<div data-testid="run-page" className="page-shell">
      <PageHeader eyebrow="Orquestração" title="Executar teste" description="Dispare um cenário individual ou uma suíte inteira no runner Playwright, reaproveitando o mesmo ambiente e dataset." meta={<>
        ...
        <form data-testid="run-form" onSubmit={handleRun} className="surface inline-form">
          <div className="field-grid">
            <label className="field">
              ...
              <select value={mode} onChange={e => setMode(e.target.value)} data-testid="select-run-mode">
                <option value="case">Cenário</option>
                <option value="suite">Suíte</option>
              </select>
            </label>

            {mode === 'case' ? (<label className="field">
                <span className="field-label">Cenário *</span>
                <select data-testid="select-case" value={selected.caseId} onChange={e => setSelected(s => ({ ...s, caseId: e.target.value }))} required>
                ...
                <select data-testid="select-suite" value={selected.suiteId} onChange={e => setSelected(s => ({ ...s, suiteId: e.target.value }))} required>
                  <option value="">Selecione...</option>
            ...
            <label className="field">
              ...
              <select data-testid="select-environment" value={selected.environmentId} onChange={e => setSelected(s => ({ ...s, environmentId: e.target.value }))} required>
                <option value="">Selecione...</option>
            ...
            <label className="field">
              ...
              <select data-testid="select-dataset" value={selected.datasetId} onChange={e => setSelected(s => ({ ...s, datasetId: e.target.value }))}>
