...
export function RunDetailPage() {
    ...
    const previewableSteps = useMemo(() => run?.stepResults.filter(step => Boolean(step.screenshotPath)) ?? [], [run]);
    ...
    useEffect(() => {
        ...
        api.get(`/test-runs/${run._id}/steps/${selectedStep.stepId}/screenshot`, {
            responseType: 'blob',
    ...
    if (!run)
        return <div className="empty-state" data-testid="not-found">Execução não encontrada</div>;
    ...
    return (<div data-testid="run-detail-page" className="page-shell">
      <PageHeader eyebrow="Detalhe da execução" title={<>Execução <code>{shortId(run._id)}</code></>} description="Acompanhe o status final, a duração total e os resultados de cada step executado pelo runner." actions={<>
            <Button onClick={handleRetryRun} disabled={!retryable || retrying} data-testid="btn-retry-run">
      ...
      {(run.error || error) && (<div className="alert alert-error" data-testid="run-error">
          {error ?? run.error}
        </div>)}
      ...
      <section className="surface">
        <div className="section-heading">
          ...
          <div className="page-meta">
            ...
            <Button type="button" variant="outline" onClick={() => setFollowLatestStep(current => !current)} disabled={run.stepResults.length === 0}>
        ...
        <div className="run-preview-shell">
          <div className="run-preview-stage">
            {previewUrl ? (<img key={previewUrl} src={previewUrl} alt={selectedStep ? `Preview do step ${selectedStep.type}` : 'Preview da execução'} className="run-preview-image"/>) : (<div className="empty-state run-preview-empty">
                <strong>
                  {previewLoading
          ...
          <aside className="run-preview-sidebar">
            ...
            {previewUrl && (<Button asChild variant="outline">
      ...
      <section className="surface">
        ...
        <div className="run-logs-grid">
          <article className="run-logs-panel">
            ...
            {recentConsoleLogs.length === 0 ? (<div className="empty-state">Nenhum log de console capturado.</div>) : (<div className="run-log-list">
                {recentConsoleLogs.map(log => (<div key={log.id} className="run-log-item">
          ...
          <article className="run-logs-panel">
            ...
            {recentNetworkLogs.length === 0 ? (<div className="empty-state">Nenhum evento de rede capturado.</div>) : (<div className="run-log-list">
                {recentNetworkLogs.map(log => (<div key={log.id + log.kind + log.timestamp} className="run-log-item">
                    <div className="run-log-row">
      ...
      <section className="surface">
        ...
        {run.stepResults.length === 0 ? (<div className="empty-state" data-testid="no-steps">
            {run.status === 'running' || run.status === 'pending'
                ? 'Aguardando execução...'
            ...
            <table className="table">
              ...
              <tbody>
                {run.stepResults.map((step, i) => (<tr key={step.stepId} data-testid={`step-result-${step.stepId}`} className={selectedStepId === step.stepId ? 'table-row-selected' : undefined} onClick={() => {
      ...
      <section className="surface">
        ...
        {auditLogs.length === 0 ? (<div className="empty-state">Nenhum evento de auditoria encontrado para este run.</div>) : (<div className="space-y-3">
...
//# sourceMappingURL=RunDetailPage.js.map