...
export function SuiteDetailPage() {
  ...
  const deleteTargets = useMemo(
    () => cases.filter(item => deleteTargetIds.includes(item._id)),
    [cases, deleteTargetIds]
  )
  ...
  return (
    <div data-testid="suite-detail-page" className="page-shell">
          ...
          selectedCaseIds.length > 0 ? (
            <>
              ...
              <Button
                type="button"
                variant="destructive"
                onClick={() => openDeleteDialog(selectedCaseIds)}
              >
      ...
      {cases.length === 0 ? (
        ...
        <>
          ...
          {cases.map(c => (
            <li
              key={c._id}
              data-testid={`case-item-${c._id}`}
              className={`list-card suite-case-card ${selectedCaseIds.includes(c._id) ? 'suite-case-card-selected' : ''}`}
            >
              <div>
      ...
      <ConfirmDialog
        ...
        tone="danger"
        busy={deleting}
        title={deleteTargetIds.length > 1 ? 'Excluir cenários selecionados?' : 'Excluir cenário?'}
        description={
          deleteTargetIds.length > 1
            ? 'Essa ação remove os cenários selecionados da suíte e limpa referências de cenário base onde for necessário.'
            : 'Essa ação remove o cenário da suíte e limpa referências de cenário base onde for necessário.'
        }
        confirmLabel={deleteTargetIds.length > 1 ? 'Excluir cenários' : 'Excluir cenário'}
        onCancel={() => {
          if (deleting) return
          setDeleteTargetIds([])
        }}
      ...
      >
        <div className="confirm-dialog-stack">
          ...
          <div className="confirm-dialog-list">
            {deleteTargets.slice(0, 5).map(item => (
              <div key={item._id} className="confirm-dialog-list-item">
                <strong>{item.name}</strong>
                <span>{item.steps.length} steps gravados</span>
              </div>
            ))}
            {deleteTargets.length > 5 && (
              <div className="confirm-dialog-list-item">
