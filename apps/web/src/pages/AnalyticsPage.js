...
export function AnalyticsPage() {
    ...
    useEffect(() => {
        Promise.all([
            ...
            api.get('/analytics/flakiness'),
            api.get('/analytics/slowest-steps'),
        ])
    ...
    const maxTotal = overview
        ...
        : 1;
    return (<div data-testid="analytics-page" className="page-shell">
      <PageHeader eyebrow="Métricas do sistema" title="Analytics" description="Taxa de sucesso, flakiness e performance dos testes ao longo do tempo."/>

      {loading ? (<div className="loading-state" data-testid="analytics-loading">Carregando...</div>) : (<div className="space-y-6">
          {/* Stats cards */}
          <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            ].map(item => (<Card key={item.label} className="bg-card/70">
          ...
          <Card className="bg-card/70">
            ...
            <CardContent>
              <div className="flex items-end gap-2 h-40" data-testid="runs-chart">
                {(overview?.runsLast7Days ?? []).map(d => (<div key={d.date} className="flex flex-col items-center gap-1 flex-1">
                    <div className="w-full flex flex-col justify-end gap-0.5" style={{ height: '100%' }}>
                      <div className="bg-red-400 w-full rounded-t-sm" style={{ height: `${(d.failed / maxTotal) * 100}%` }} title={`Failed: ${d.failed}`}/>
                      <div className={`bg-green-400 w-full ${d.failed === 0 ? 'rounded-t-sm' : ''}`} style={{ height: `${(d.passed / maxTotal) * 100}%` }} title={`Passed: ${d.passed}`}/>
                    </div>
          ...
          <Card className="bg-card/70">
            ...
            <CardContent>
              {flakiness.length === 0 ? (<div className="empty-state" data-testid="flakiness-empty">Nenhum dado disponível.</div>) : (<div className="overflow-x-auto rounded-xl border border-border/70">
                  <table className="table" data-testid="flakiness-table">
                    ...
                    <tbody>
                      {flakiness.map(entry => (<tr key={entry.caseId}>
          ...
          <Card className="bg-card/70">
            ...
            <CardContent>
              {slowestSteps.length === 0 ? (<div className="empty-state" data-testid="slowest-empty">Nenhum dado disponível.</div>) : (<div className="overflow-x-auto rounded-xl border border-border/70">
                  <table className="table" data-testid="slowest-steps-table">
                    ...
                    <tbody>
                      {slowestSteps.map(step => (<tr key={step.stepId}>
                          <td>
