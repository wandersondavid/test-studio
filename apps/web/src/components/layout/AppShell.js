...
import { BarChart3, Bell, Blocks, Clock, Clock3, Gauge, LogOut, MonitorPlay, PlayCircle, Search, ServerCog, Sparkles, Users, } from 'lucide-react';
...
function resolveCurrentLabel(pathname) {
    if (pathname.startsWith('/cases/'))
        return 'Builder';
    if (pathname.startsWith('/suites/'))
        return 'Detalhe da suíte';
    if (pathname.startsWith('/history/'))
        return 'Detalhe da execução';
      ...
      <div className="grid min-h-screen lg:grid-cols-[280px_minmax(0,1fr)]">
        <aside className="border-r border-border/70 bg-card/35 backdrop-blur-xl" data-testid="sidebar">
          <div className="flex h-full flex-col gap-6 p-4">
            ...
            return (<NavLink key={item.to} to={item.to} end={item.to === '/'} className={({ isActive }) => cn('group flex items-start gap-3 rounded-xl border px-3 py-3 transition-colors', isActive
        ...
        <div className="min-w-0">
          <header className="sticky top-0 z-20 border-b border-border/70 bg-background/85 backdrop-blur-xl">
            <div className="flex flex-col gap-4 px-5 py-4 lg:px-8 xl:flex-row xl:items-center xl:justify-between">
              ...
              <label className="relative mx-auto flex w-full max-w-2xl items-center" aria-label="Pesquisar workspace">
                ...
                <Input type="text" readOnly value="" placeholder="Pesquisar cenários, execuções e ambientes" className="h-11 rounded-full border-border/80 bg-card/70 pl-10 pr-4"/>
              </label>
          ...
          <main className="min-w-0 px-5 py-6 lg:px-8">
            {user && (<div className="mb-5 flex flex-wrap items-center gap-2">
