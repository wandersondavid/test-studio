...
export function AiStepsModal({ open, onClose, onStepsGenerated }: AiStepsModalProps) {
  const [description, setDescription] = useState('')
  const [baseURL, setBaseURL] = useState('')
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [generatedSteps, setGeneratedSteps] = useState<TestStep[] | null>(null)
        ...
        <div className="confirm-dialog-body" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {error && (
            <p
              ...
              style={{ fontSize: 13, color: '#dc2626', margin: 0, padding: '8px 10px', background: '#fef2f2', borderRadius: 6, border: '1px solid #fecaca' }}
            >
              {error}
            </p>
          )}
          ...
          {generatedSteps && generatedSteps.length > 0 && (
            <div data-testid="ai-steps-preview">
              ...
              >
                {generatedSteps.map((step, idx) => (
                  <li
                    key={step.id}
                    data-testid={`ai-step-preview-${idx}`}
                    style={{
                      padding: '6px 10px',
                      borderRadius: 6,
                      background: '#f8fafc',
        ...
        <div className="confirm-dialog-actions">
          ...
          {generatedSteps && generatedSteps.length > 0 ? (
            <Button
              type="button"
              onClick={handleAdd}
              data-testid="ai-modal-add-steps"
            >
              Adicionar ao Caso
            </Button>
          ) : (
