...
export function ExportModal({ open, caseId, caseName, onClose }) {
    ...
    async function handleDownload() {
        ...
        try {
            ...
            const res = await api.get(`/test-cases/${caseId}/export${params}`, {
                responseType: 'text',
            });
    ...
    return createPortal(<div className="confirm-dialog-backdrop" role="presentation">
      <div className="confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="export-modal-title" data-testid="export-modal">
        ...
        <div className="confirm-dialog-body">
          ...
          <select id="export-env-select" data-testid="export-env-select" value={selectedEnvId} onChange={e => setSelectedEnvId(e.target.value)} style={{ width: '100%', padding: '6px 8px', borderRadius: 6, border: '1px solid #d1d5db', fontSize: 14 }}>
            <option value="">Sem ambiente</option>
            {environments.map(env => (<option key={env._id} value={env._id}>
                {env.name} ({env.baseURL})
              </option>))}
          </select>
          {downloadError && (<p style={{ marginTop: 8, fontSize: 13, color: '#dc2626' }} data-testid="export-error">{downloadError}</p>)}
        </div>
        <div className="confirm-dialog-actions">
          <Button type="button" variant="outline" onClick={onClose} disabled={loading}>
            Cancelar
          </Button>
          <Button type="button" data-testid="btn-download-spec" onClick={handleDownload} disabled={loading}>
