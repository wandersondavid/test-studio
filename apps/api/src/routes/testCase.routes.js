...
testCaseRouter.get('/', async (req, res, next) => {
    try {
...
testCaseRouter.post('/bulk-delete', async (req, res, next) => {
    try {
        ...
        await Promise.all(existingCases.map(item => (auditLogService.create({
            entityType: 'case',
            entityId: item.id,
            action: 'case_deleted',
...
testCaseRouter.get('/:id/export', async (req, res, next) => {
    try {
...
testCaseRouter.get('/:id/executable', async (req, res, next) => {
    try {
...
testCaseRouter.get('/:id', async (req, res, next) => {
    try {
...
testCaseRouter.post('/', async (req, res, next) => {
    try {
...
testCaseRouter.put('/:id', async (req, res, next) => {
    try {
        ...
        if (current) {
            ...
            for (const step of item.steps) {
                ...
                if (stepSignature(previous) !== stepSignature(step)) {
                    await auditLogService.create({
                        entityType: 'case',
                        entityId: item.id,
                        action: 'step_updated',
...
testCaseRouter.delete('/:id', async (req, res, next) => {
    try {
