...
function buildEventForStatus(status) {
    if (status === 'passed')
        return ['on_pass', 'always'];
    if (status === 'failed' || status === 'error')
        return ['on_fail', 'always'];
    return [];
}
function buildSlackPayload(run, caseName) {
...
function buildWebhookPayload(run, caseName) {
    return {
        runId: run.id,
        status: run.status,
        caseName,
...
export async function sendNotification(run, channels, caseName) {
    const matchingEvents = buildEventForStatus(run.status);
    const activeMatching = channels.filter(ch => ch.isActive && ch.events.some(ev => matchingEvents.includes(ev)));
    await Promise.allSettled(activeMatching.map(async (channel) => {
        try {
